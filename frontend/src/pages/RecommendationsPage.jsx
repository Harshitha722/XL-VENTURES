...
function RecommendationsPage() {
    ...
    async function approveRecommendation(item) {
        ...
        try {
            await api.post("/memory/approve-recommendation", {
            ...
            setError(
                status === 404
                    ...
                    : message || "Unable to save approval to memory."
            );
        }
        finally {
            setSaving((current) => ({
            ...
            {!analysis?.timestamp ? (
                ...
                <section className="recommendation-list">
                    {explanations.map((item) => {
                        const isApproved = Boolean(approved[item.recommendation]);
                        const isSaving = Boolean(saving[item.recommendation]);
